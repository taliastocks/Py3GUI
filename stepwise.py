...
def matlab_stepwisefit(allx, y, inmodel = [], penter = 0.05, premove = 0.10,
    display = False, maxiter = np.Inf, keep = [], scale = False):
    def to_array(text):
        original = text
    ...
    h.Execute("format long")
    matlab_allx = "];".join(str(allx.tolist()).split("],"))
    print h.Execute("allx = %s;" % matlab_allx)
    matlab_y = "];".join(str(y.tolist()).split("],"))
    print h.Execute("y = %s;" % matlab_y)
    print h.Execute("[B, SE, PVAL, in] = stepwisefit(allx, y, 'display', 'off', 'maxiter', 60);")
    matlab_B = to_array(h.Execute("disp(B);"))
    matlab_SE = to_array(h.Execute("disp(SE);"))
    matlab_PVAL = to_array(h.Execute("disp(PVAL);"))
    matlab_inmodel = to_array(h.Execute("disp(in);"))
    return matlab_B, matlab_SE, matlab_PVAL, matlab_inmodel > 0, None, None, None

#stepwisefit = matlab_stepwisefit
...
def test():
    ...
    h = win32com.client.Dispatch("matlab.application")
    h.Execute("format long")
    rows = np.random.randint(2, 2000)
    cols = np.random.randint(1, 800)
    print (rows, cols)
    allx = np.random.randint(-10, 10, (rows, cols))
    y = np.random.randint(0, 2, (rows, 1)) * 2 - 1
    allx += y
    #print y
    matlab_allx = "];".join(str(allx.tolist()).split("],"))
    h.Execute("allx = %s;" % matlab_allx)
    #print `allx`
    matlab_y = "];".join(str(y.tolist()).split("],"))
    h.Execute("y = %s;" % matlab_y)
    t = time.time()
    h.Execute("[B, SE, PVAL, in] = stepwisefit(allx, y, 'display', 'off', 'maxiter', 60);")
    ...
    B, SE, PVAL, inmodel, stats, nextstep, history = \
        stepwisefit(allx, y, display = False, maxiter = 60)
    print "NumPy:", time.time() - t
    inmodel = np.asarray(inmodel, dtype = float)
    difference = max(
        abs(matlab_B - B.ravel()).max(),
        abs(matlab_SE - SE.ravel()).max(),
        abs(matlab_PVAL - PVAL.ravel()).max(),
        abs(matlab_inmodel - inmodel.ravel()).max()
    )
    print difference
    if difference > 1e-11:
        print matlab_inmodel
        print inmodel
